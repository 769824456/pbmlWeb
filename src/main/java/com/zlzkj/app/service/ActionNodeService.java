...
//			return -1;
	...
	public List<Row> getTopNodeList() {
		String sql = SQLBuilder.getSQLBuilder(ActionNode.class)
				.fields("*").where("pid=0 and is_menu=1 and is_show=1").order("sort_id", "ASC").buildSql();
	...
	public List<Row> getLeftNodeList(Integer topNodeId) {
		...
		String sql = sb.fields("*").where("pid="+topNodeId+" and level=2 and is_menu=1 and is_show=1").order("sort_id", "ASC").buildSql();
		List<Row> list = sqlRunner.select(sql);
